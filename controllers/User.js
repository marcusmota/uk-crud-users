...
const getAll = async (req, res) => {
  const {
    limit, skip, givenName, familyName, email,
  } = req.query;
...
const postUser = async (req, res) => {
  req.assert('givenName')
    .isLength({ min: 1, max: 100 }).withMessage('the given name field is required').trim()
    .escape();
  req.assert('familyName')
    .isLength({ min: 1, max: 100 }).withMessage('the family name field is required').trim()
    .escape(),
  req.assert('email')
    .isLength({ min: 1, max: 100 }).withMessage('the email field is required')
    .isEmail()
    .withMessage('you must insert a valid email address');

  try {
    await req.asyncValidationErrors();

    if (await userRepository.couldStoreEmailOnCreate(req.body.email) === false) {
      return res.status(422).send(parseArrayToObject([{ param: 'email', msg: 'the email address is already taken' }]));
    }
...
const putUserById = async (req, res) => {
  ...
  try {
    ...
    if (await userRepository.couldStoreEmailOnUpdate(id, req.body.email) === false) {
      return res.status(422).send(parseArrayToObject([{ param: 'email', msg: 'the email address is already taken' }]));
    }
